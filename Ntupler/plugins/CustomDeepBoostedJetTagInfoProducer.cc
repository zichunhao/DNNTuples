...
class CustomDeepBoostedJetTagInfoProducer : public edm::stream::EDProducer<> {
public:
  explicit CustomDeepBoostedJetTagInfoProducer(const edm::ParameterSet &);
  ~CustomDeepBoostedJetTagInfoProducer() override;
...
const std::vector<std::string> CustomDeepBoostedJetTagInfoProducer::particle_features_{
...
const std::vector<std::string> CustomDeepBoostedJetTagInfoProducer::sv_features_{
...
CustomDeepBoostedJetTagInfoProducer::CustomDeepBoostedJetTagInfoProducer(const edm::ParameterSet &iConfig)
...
CustomDeepBoostedJetTagInfoProducer::~CustomDeepBoostedJetTagInfoProducer() {}

void CustomDeepBoostedJetTagInfoProducer::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
  ...
  descriptions.add("pfCustomDeepBoostedJetTagInfos", desc);
}

void CustomDeepBoostedJetTagInfoProducer::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
...
void CustomDeepBoostedJetTagInfoProducer::fillParticleFeatures(DeepBoostedJetFeatures &fts, const reco::Jet &jet) {
  ...
  for (const auto &cand : daughters) {
    ...
    fts.fill("pfcand_energy", pfp4.E());
  }
}

void CustomDeepBoostedJetTagInfoProducer::fillSVFeatures(DeepBoostedJetFeatures &fts, const reco::Jet &jet) {
...
DEFINE_FWK_MODULE(CustomDeepBoostedJetTagInfoProducer);
